...
import { Menu, X, ArrowRight, Dumbbell, Flower2, Music, Waves, User, Trophy } from "lucide-react";
...
const NAV_ITEMS = [
  { 
    ...
    dropdown: [
      ...
      { label: "Swimming Academy", href: "/solutions/swim", icon: Waves, desc: "Pool schedules, coaches, lane booking & memberships", color: "text-cyan-500", bg: "bg-cyan-50" },
    ]
  },
  ...
  { 
    ...
    dropdown: [
      ...
      { label: "Gym Digital Readiness", href: "/tools/gym-digital-readiness-score" },
...
export default function Header() {
  ...
  const getLinkStyles = (href: string) => {
    ...
    const baseStyle = "text-sm tracking-wide font-medium px-4 py-2 relative rounded-full transition-all duration-200 ease-out hover:-translate-y-0.5 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF5C73]/40";
    const homeStyle = isHome && !isScrolled ? "text-slate-200 hover:text-white hover:bg-white/10" : isHome && isScrolled ? "text-slate-300 hover:text-white hover:bg-white/10" : "text-slate-600 hover:text-slate-900 hover:bg-slate-100";
    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${getNavStyles()}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8 h-full flex items-center justify-between">
          ...
          {NAV_ITEMS.map((item) => (
            item.dropdown ? (
              <div key={item.href} className="relative group">
                ...
                <div className={`absolute top-full pt-4 opacity-0 invisible translate-y-2 group-hover:translate-y-0 group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 ${item.megaMenu ? 'left-1/2 -translate-x-[20%]' : 'left-1/2 -translate-x-1/2'}`}>
                   {item.megaMenu ? (
                     <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-100 w-[780px] flex flex-col overflow-hidden">
                       <div className="p-6 grid grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                         {item.dropdown.map((sub: any) => (
                           <Link key={sub.label} href={sub.href} className="flex items-start gap-4 group/item relative rounded-2xl p-2 -m-2 transition-all duration-200 hover:bg-slate-50 hover:shadow-sm">
                             <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center transition-transform duration-200 group-hover/item:scale-105 ${sub.bg}`}>
                       ...
                       {/* Bottom CTA */}
                       <div className="bg-slate-50/90 border-t border-slate-100 p-6 flex items-center justify-between">
                         <div>
                     ...
                     <div className="bg-white/95 backdrop-blur-xl rounded-xl shadow-xl border border-slate-100 py-2 w-56 flex flex-col overflow-hidden">
                       {item.dropdown.map(sub => (
                          <Link key={sub.label} href={sub.href} className="px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-[#FF5C73] font-medium transition-all border-l-2 border-transparent hover:border-[#FF5C73]">
                            {sub.label}
                          </Link>
                       ))}
                     </div>
        ...
        <button
          ...
          className={`lg:hidden p-2 rounded-lg transition-all duration-200 ${isHome ? 'text-white hover:bg-white/10 hover:scale-105' : 'text-slate-900 hover:bg-slate-100 hover:scale-105'}`}
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`text-base font-semibold tracking-wide py-2 px-4 rounded-xl transition-colors ${
                  isActive ? "text-[#FF5C73] bg-rose-50 underline decoration-2 underline-offset-4" : isHome ? "hover:bg-white/5" : "hover:bg-slate-50"
                }`}
              >
                {item.label}
              </Link>
