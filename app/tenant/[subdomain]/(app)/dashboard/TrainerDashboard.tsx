...
export async function TrainerDashboard({ subdomain }: { subdomain: string }) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--color-muted-foreground)" }}>
            Manage your classes and assigned members.
          </p>
        </div>
      </div>
      <div className="mt-8 p-8 bg-white rounded-2xl shadow-sm border border-slate-100">
        <p className="text-slate-600">Your assigned members and schedule will appear here.</p>
      </div>
    </div>
  );
}
