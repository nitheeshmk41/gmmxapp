...
export function MemberFormPage({ plans, mode, member }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (result?.error) {
        ...
        router.push("/owner/dashboard/members");
      }
    });
  }
  ...
  return (
    <div className="max-w-2xl animate-in">
      <Link href="/owner/dashboard/members" className="flex items-center gap-2 text-sm mb-5 transition-colors" style={{ color: "var(--color-muted-foreground)" }}>
        <ArrowLeft size={15} />
        Back to Members
      </Link>
      ...
      <div className="p-6 rounded-2xl" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", boxShadow: "var(--shadow-card)" }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3 pt-2" style={{ borderTop: "1px solid var(--color-border-muted)" }}>
            <Link href="/owner/dashboard/members" className="flex-1 py-3 rounded-xl text-sm font-medium text-center" style={{ background: "var(--color-border)", color: "var(--color-foreground)" }}>
              Cancel
            </Link>
